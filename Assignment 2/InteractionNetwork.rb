...
# this class will contain all the interaction networks of'ArabidopsisSubNetwork_GeneList.txt' genes, as instances
...
class InteractionNetwork
  ...
  # it will be an array containing all the AGIs codes of the members of the interaction network
  attr_accessor :members
  # 3. KEGG pathways
  # it will be a hash to store all possible KEGG ID/pathways that may have each member of the network
  attr_accessor :kegg_pathway
  # 4. GO Terms
  # it will be a hash to store all possible GO ID/terms that may have each member of the network
  attr_accessor :go_terms
  
  ## initializing instance variables ##
  def initialize(params = {})
    # each time an instance is created with .new() method...
    # 1. assigning a number to the network to identify them
    ...
    # 3. anotation of KEGG ID and pathway name of all memebers in the interaction network --> hash
    # it takes members as argument
    @kegg_pathway = annotate_kegg(members = @members)
    # 4. anotation of GO ID and GO terms of all members of the interaction network --> hash
    # it also takes members as argument
    @go_terms = annotate_go(members = @members)
    # 5. the total number of instances increases by 1
    @@number_networks +=1
    # 6. the instance is stored in the instances array
    @@all_networks << self 
  ...
  def self.getting_agi(file)
    ...
    File.open(file).each do |gene|
      # pass the content of the line to @@agis array class variable
      @@agis << gene.strip.downcase
    end
    return @@agis
  end
  
  ## 2. attempting to contact the webs and handling possible errors ##
  ...
  ## 3. searches for interactions in BAR database ##
  def self.search_protein_int() 
    # creating an empty class variable hash to store interactions
    ...
    agis = @@agis
      ...
      if res
        ...
        body.each do |int| 
          ...
          next if array[2].match(/(A[Tt]\w[Gg]\d+)/) == nil
          next if array[3].match(/(A[Tt]\w[Gg]\d+)/) == nil 
          # saving data in variables and converting to string
          int_1 = array[2].match(/(A[Tt]\w[Gg]\d+)/)[0].to_s
          int_2 = array[3].match(/(A[Tt]\w[Gg]\d+)/)[0].to_s
          ...
          # 3. if interactor 1 is the AGI used for search and interactor 2 is not the AGI used for search and interactor 2 is not in the AGIs list...
            ...
            if res 
              ...
              body2.each do |int2| 
                ...
                next if array[2].match(/(A[Tt]\w[Gg]\d+)/) == nil
                next if array[3].match(/(A[Tt]\w[Gg]\d+)/) == nil
                int_12 = array[2].match(/(A[Tt]\w[Gg]\d+)/)[0].to_s
                int_22 = array[3].match(/(A[Tt]\w[Gg]\d+)/)[0].to_s
                ## adding the condition that the new interactor is in the AGIs list and is not the same at the first one used for searching
          ...
          # 3. if interactor 2 is the AGI used for search and interactor 1 is not the AGI used for search and interactor 1 is not in the AGIs list...
            ...
            if res 
              ...
              body3.each do |int3| 
                ...
                next if array[2].match(/(A[Tt]\w[Gg]\d+)/) == nil
                next if array[3].match(/(A[Tt]\w[Gg]\d+)/) == nil
                int_123 = array[2].match(/(A[Tt]\w[Gg]\d+)/)[0].to_s
                int_223 = array[3].match(/(A[Tt]\w[Gg]\d+)/)[0].to_s
  ...
  def self.create_networks()
    ...
    @@interactions.each do |key,value|
      ...
      number += 1
      # store in array members of indirect interactions: values = array, key = string
      ...
      # store in array members of direct interactions: values = string, key = string
  ...
  ## 1. annotate KEGG ID and KEGG pathways for members of a sinlge network ##
  def annotate_kegg(members)
    ...
    members.each do |member|
      # creating an empty array to store info about a member (key)
      kegg_annotations[member] = Array.new
      res = InteractionNetwork.fetch("http://togows.org/entry/genes/ath:#{member}/pathways.json")
      if res 
        # parse a JSON document into a ruby data structure
        data = JSON.parse(res.body)
        # data[0] is a hash 
        # iterating for each entry of this hash
        # key and value
        data[0].each do |kegg_id,kegg_p|
          # saving each KEGG ID and KEGG pathway as a value (array) of the key
  ...
  def annotate_go(members)
    ...
    members.each do |member|
      go_annotations[member] = Array.new
      res = InteractionNetwork.fetch("http://togows.org/entry/ebi-uniprot/#{member}/dr.json")
      if res 
        data = JSON.parse(res.body)
        # want to take the key "GO"
        # and iterate over the values (lists)
        data[0]["GO"].each do |go|
          ...
          # using regex to only take the number and the description as variables 
          go_id = go[0].match(/(\d+)/)[1]
